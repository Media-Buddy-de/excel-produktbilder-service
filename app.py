...
def process_excel():
    ...
    try:
        ...
        # Spalte "Artikelnummer_1" finden
        header_row = ws[1]
        artikel_col = None
        
        for cell in header_row:
            if cell.value and "Artikelnummer_1" in str(cell.value):
        ...
        if not artikel_col:
            return jsonify({
                "success": False,
                "error": "Spalte 'Artikelnummer_1' nicht gefunden"
            }), 400
        
        # Neue Spalte für Bilder
        bild_col = ws.max_column + 1
        ws.cell(row=1, column=bild_col, value="Produktbild")
        ...
        return jsonify({
            ...
            "output_filename": f"bearbeitet_{original_filename}",
            "stats": stats,
            "message": f"Erfolgreich {stats['successful_images']} von {stats['total_rows']} Bildern eingefügt"
        }), 200
